...
{
    internal sealed class InboxRepository(ICatalogUnitOfWork unitOfWork) : IInboxRepository
    {
        public Task InsertAsync(IntegrationEvent integrationEvent, CancellationToken cancellationToken)
        {
            const string sql = """
                INSERT INTO catalog."InboxMessages"("Id", "CorrelationId", "Type", "Content", "OccurredOn", "RetryCount", "IsPermanentFailure")
        ...
        public Task<bool> IsProcessedAsync(InboxMessageConsumer inboxMessageConsumer, CancellationToken cancellationToken)
        {
            const string sql = """
                SELECT EXISTS (
                    SELECT 1 FROM catalog."InboxMessageConsumers"
        ...
        {
            const string sql = """
                INSERT INTO catalog."InboxMessageConsumers" ("Id", "InboxMessageId", "Name")
