...
namespace Modules.Launches.Infrastructure.Inbox
{
    ...
    {
        private const string SubscriptionName = "launches.sub";
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            while (!stoppingToken.IsCancellationRequested)
            {
                try
                {
                    ...
                    logger.LogInformation("[Launches] Subscribed to integration event topics");
                ...
                catch (Exception ex)
                {
                    logger.LogWarning(ex,
                        "[Launches] Failed to subscribe to Service Bus. Retrying in {Delay}s...",
            ...
            await DisposeSubscriptionsAsync();
            logger.LogInformation("[Launches] Unsubscribed from integration event topics");
        }
        ...
        private async Task HandleAsync(ConsumedMessage message, CancellationToken cancellationToken)
        {
            ...
            {
                logger.LogError(ex,
                    "[Launches] Failed to persist integration event {MessageType} [{MessageId}] to inbox",
                    message.MessageType,
                    message.MessageId);
